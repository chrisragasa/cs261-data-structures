...
TYPE getMinHeap(DynArr *heap)
{
	...
	assert(heap->size != 0);
  	...
  	//Lowest priority (min) node is at index 0
  	return heap->data[0];
}
...
void addHeap(DynArr *heap, TYPE node)
{
    ...
    //Find the next open spot in the array
    ...
    //Perculate up
    while(pos != 0) {
    	//Compute the parent of the position
        parent = (pos - 1)/2;

        //If the value at pos is less than the value at parent, swap the two and set the new position to the parent
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
  	int leftChild = 2 * pos + 1; //stores index of left child (formula)
  	int rightChild = 2 * pos + 2; //stores index of right child (formula)

  	//if value rightChild is less than max (an empty index), there exists a rightChild based
  	//off of the formula. There is also a left child because in a heap, left child is 
  	//always filled before right
  	if(rightChild < max) {
  		//take the index of the child with the smaller value
  		int smallestChild = _smallerIndexHeap(heap, leftChild, rightChild);

  		//if the value of the smallestChild is smaller than the value at pos,
  		//swap the two then recursively call adjust heap on the child
  	...
  	//if value leftChild is less than max (an empty index), there exists a leftChild
  	else if(leftChild < max) {
  		 //if the value of the leftChild is smaller than the value at pos,
  		//swap the two, then recursively call adjust heap on the leftChild
  	...
  	//otherwise, there are no children. Adjust heap is done, do nothing else.
}
...
{
	//Check that the heap exists and isn't empty
	assert(heap != 0);
	assert(heap->size != 0);

  	//Switch min (first) and last element
...
void _buildHeap(DynArr *heap)
{
	...
	//Compute the index of the first non-leaf node (first root of a subtree)
	int indexRootFirstSubtree = (heap->size / 2) - 1;

	//While you're not at the root, adjust the heap from indexRootFirstSubtree to max
	//Then decrement i and repeat process until you run process the root
	while(indexRootFirstSubtree >= 0) {
		_adjustHeap(heap, heap->size, indexRootFirstSubtree);
		indexRootFirstSubtree--;
	}	
}
/* 
...
void sortHeap(DynArr *heap)
{
	...
	//Compute the index of the last node in the heap
	int lastIndex = heap->size - 1;

	//While lastIndex isn't at the root, swap last and first, then adjust heap from root up to last
	while(lastIndex >= 0) {
